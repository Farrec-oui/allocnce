...
export default function ModalCreate({ open, onClose, onSuccess, allocs = [] }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      onSuccess("Allocation générée avec succès.");
    } catch (err) {
  ...
  return (
    <ModalBase open={open} onClose={onClose} title="Créer une allocation" subtitle="Allocation depuis un PDF EasyJet">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          {label("PDF allocation EasyJet", true)}
          <DropZone label="FlightAllocationReport.pdf" onChange={setPdf} />
        </div>
        ...
        >
          {loading ? "Génération en cours…" : "Créer l'allocation"}
        </button>
      </form>
    </ModalBase>
  );
